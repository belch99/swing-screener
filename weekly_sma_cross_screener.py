"""
Weekly 20/50 SMA Cross Screener
--------------------------------
Scans NASDAQ + NYSE listed stocks for a weekly close where the 20-week SMA
crosses above the 50-week SMA (a "golden cross" on the weekly timeframe).

Filters applied:
  - Last close > $5
  - 20-week average volume > 500,000
  - Cross must have happened on the MOST RECENTLY CLOSED weekly bar
...
RUN:
    python weekly_sma_cross_screener.py

    Optional flags:
    python weekly_sma_cross_screener.py --tickers all_tickers.csv --min-price 5 --min-avg-vol 500000

Output:
    Prints matches to the console and saves them to weekly_cross_results.csv
"""
...
LOOKBACK_PERIOD = "2y"    # enough weekly bars to compute a 50-week SMA reliably
...
def screen_batch(tickers, min_price, min_avg_vol):
    ...
    for t in tickers:
        try:
            ...
            crossed_today = sma20.iloc[-2] <= sma50.iloc[-2] and sma20.iloc[-1] > sma50.iloc[-1]
...
def main():
    parser = argparse.ArgumentParser(description="Weekly 20/50 SMA cross screener")
    ...
    parser.add_argument("--output", default="weekly_cross_results.csv")
